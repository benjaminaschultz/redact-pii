...
username_regex = re.compile('(user( ?name)?|login): \S+', re.IGNORECASE)
password_regex = re.compile('(pass(word|phrase)?|secret): \S+', re.IGNORECASE)
credentials_regex = re.compile('(login( cred(ential)?s| info(rmation)?)?|cred(ential)?s) ?:\s*\S+\s+\/?\s*\S+', re.IGNORECASE)
company_regex = re.compile('([A-Z&][\w,]* )+(I[Nn][Cc](orporated)?|C[Oo](rp(oration)?)?|LLP|llc|LLC|plc|gmbh)\.?', re.IGNORECASE)
